...
module.exports.run = async function({ api, event, args }) {
  ...
  try {
    if (isLink) {
      ...
      const res = await axios.get(`https://apichatbot.sumiproject.io.vn/imgur?link=${encodeURIComponent(linkUp)}`);
      const link = res.data.uploaded.image;
      const response = await axios.get(`https://apichatbot.sumiproject.io.vn/lamnet?link=${link}`, { responseType: "arraybuffer" });
      api.sendMessage("Đang load, chờ tí nhé....!", event.threadID);
      fs.writeFileSync(__dirname + `/cache/netanh.png`, Buffer.from(response.data, "binary"));
    }
